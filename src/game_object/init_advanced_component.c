...
{
    prop_t prop = my_getnbr(buffer);
    char **parsed_buffer = my_str_to_word_array(buffer, " :;");
    sfBool error = sfFalse;
    int size = 0;

    if (!parsed_buffer || !prop || my_array_len((void **)parsed_buffer) != 4)
        return (sfFalse);
...
sfBool init_image(game_obj_t *obj, char *buffer)
{
    ...
    if (!parsed_buffer || !prop || my_array_len((void **)parsed_buffer) != 3)
        return (sfFalse);
